...
def list_doctor_prevent_records_endpoint(
...
@router.get("/export")
def export_doctor_prevent_records_endpoint(
    date_from: date | None = Query(default=None),
    date_to: date | None = Query(default=None),
    physician_name: str | None = Query(default=None),
    diabetes: bool | None = Query(default=None),
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="doctor_prevent_records_export.csv"'},
    )


@router.get("/export.xlsx")
def export_doctor_prevent_records_xlsx_endpoint(
    date_from: date | None = Query(default=None),
    date_to: date | None = Query(default=None),
    physician_name: str | None = Query(default=None),
    diabetes: bool | None = Query(default=None),
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="doctor_prevent_records_export.xlsx"'},
    )


@router.get("/{record_id}", response_model=PreventRecordDetailResponse)
def get_doctor_prevent_record_detail_endpoint(
    record_id: UUID,
    db: Session = Depends(get_db),
    current_user: AuthenticatedUser = Depends(require_doctor),
) -> PreventRecordDetailResponse:
    ...
    record = get_prevent_record_detail(
        db=db,
        record_id=record_id,
        owner_doctor_id=current_user.doctor_profile.id,
        visibility_scope="doctor_private",
